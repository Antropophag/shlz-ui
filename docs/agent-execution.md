...
A guarded packet may declare `maxInitialContextBytes` as a positive integer. Immediately before worker launch, the harness resolves the complete fresh-ledger phase capsule and sums every `readNow` source byte. A total above the packet maximum fails closed with the measured total, maximum, and largest contributors. The harness never truncates or drops a declared source, contract, obligation, finding, or evidence pointer to make a packet fit: narrow an over-broad `contextSources` declaration or deliberately revise the reviewed packet budget. Plans without the field remain valid and unbudgeted.
...
The starting context hypotheses are 40–70k normal, 60–80k pressure, 80–100k red zone, and above 100k a strong decomposition/fresh-context signal. Runtime tokens take precedence when a trustworthy source exposes them. `telemetry-summary` keeps raw input, cached input, uncached input, and output separate when the adapter supplied them and attributes usage by packet, phase, logical session, physical runtime, and attempt. Historical events missing a field report it as `unavailable`; bytes and aggregate token arithmetic do not fill the gap. Without comparable trusted before/after telemetry, keep usage, peak active context, and total improvement `unavailable`; use labeled non-additive proxies such as repeated/unique reads, repeated discovery commands, command/output volume, phase changes, handoff bytes, and irrelevant-output accumulation. Content replaced by a digest is `not reread`, not proven prevented processing or retention. Validation/CI generation and raw-log retention remain incurred even when later phases consume a compact index. Context relevance is only the observed ratio of explicitly classified reads, never a semantic quality score. In the red zone add no new scope; finish/handoff, compact only when continuity is valuable, or start fresh.
