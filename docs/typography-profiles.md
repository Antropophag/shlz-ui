...
SHLZ UI has one component system and two supported typography profiles:

- **Golos Text** is the default and the authoritative profile for Figma/source fidelity.
- **Fira Sans** is the supported alternative for denser corporate interfaces.

No attribute is required for existing consumers. This preserves the original behavior:
...
Enable Fira centrally on the document or on any subtree:
...
An explicit `data-shlz-font="golos"` is also supported. A nested profile overrides its
...
The production CSS intentionally contains no CDN request and no embedded font binary.
...
Use `font-display: swap` (or the consumer's established loading policy), serve WOFF2
locally, and include Cyrillic and Latin coverage. Both stacks end in explicit system-sans
fallbacks for resilient rendering.

## Geometry contract

Profiles may differ in family and, if evidence later requires it, semantic size, weight,
