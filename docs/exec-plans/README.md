...
Before implementation, `route-check` validates semantic route evidence and `implementation-preflight` binds it to requirements intent/change/decision ownership/readiness plus the real task-branch/base state. Before completion, `route-conformance` binds discovered semantic surface to the actual fixed-base diff and `delivery-check` queries Git/GitHub to require local HEAD, the task-branch upstream, and the open PR head to be the same commit targeting the default branch. Route/discovery JSON is operational guard evidence, not normative requirements; OpenSpec remains authoritative.
