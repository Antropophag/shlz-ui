...
Finding `tabs-controller-lifecycle-and-aria-validation` (P1) was closed: repeated enhancement now reuses the live controller, destroy releases the root, initial selection excludes both native and ARIA-disabled tabs, and missing/duplicate/cross-root tab↔panel IDs fail fast. Browser evidence covers click/ArrowLeft/ArrowRight/Home/End, disabled skip, one selected/tab-stop tab, programmatic enhancement, teardown, re-enhancement and malformed ARIA. Consumer integration is not applicable because no application/Data Workspace Tabs consumer exists. Narrow evidence explicitly tests a consumer-provided horizontal overflow container; it does not invent mobile collapse/wrap behavior.
...
| Tabs       | three source families  | ARIA relationships/export        | keyboard/idempotence/teardown/ID validation | new active/focus/variants snapshot           | N/A: no consumer          | consumer-owned scroll explicitly exercised |
