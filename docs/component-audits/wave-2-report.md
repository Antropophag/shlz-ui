...
- Evidence: source tests, occurrence guard, native runtime/programmatic checks, Data Workspace search, narrow Fira containment and focused state snapshot.
...
- Evidence: source matrix test, occurrence guard, multiline/programmatic runtime, explicit invalid/described relationship, narrow content stress and focused state snapshot.
- Findings/fixes: old evidence had runtime, focused visual and responsive gaps (P2, closed). The previous broad fixture did not prove counter ownership or error relationship. No production CSS/API fix was required.
- Status: `VERIFIED`.
...
- Evidence: source geometry tests, occurrence guard, exact two-event pointer/Space lifecycle, programmatic/mixed state, Data Workspace row/select-all/clear behavior and focused snapshot.
...
- Evidence: source tests, occurrence guard, native group keyboard/exclusion/programmatic checks and focused group snapshot.
...
- Ownership: native checkbox with `role=switch` owns focus, checked, Space and events; SHLZ owns static appearance and the documented 120ms thumb transition; consumer owns persistence/failure handling.
- Evidence: complete source matrix tests, occurrence guard, semantic/keyboard/programmatic checks, reduced-motion CSS and focused two-size snapshot.
