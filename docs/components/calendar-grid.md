...
Wrap an accessible `table` in `.shlz-calendar-grid`, label every row and date with native `th` elements, and connect data cells with `headers`. Optional month or period bands use an additional header row with native `<th scope="colgroup" colspan="…">`; individual dates remain separate `<th scope="col">` headers beneath it. Set `data-shlz-calendar-grid-state` on both a date header and every corresponding body cell: `past` uses a muted filled header over base body cells, `today` uses an accent-tinted header and subtle fill through the whole body column, `future` uses a bordered base header over base body cells, and `unavailable` uses the source hatch across each marked cell. The library paints these states but does not calculate them. Unavailable dates also need visible or assistive text explaining why. Generic item tones are presentation-only: `accent`, `success`, `warning`, and `danger` carry no business meaning.
