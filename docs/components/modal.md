...
```html
<button type="button" data-shlz-modal-trigger="example-modal">Open</button>
...
>
  <div class="shlz-modal__surface">
    <header class="shlz-modal__header">
      ...
      <button
        type="button"
        class="shlz-modal__close"
        data-shlz-modal-close
        aria-label="Close"
      >
        ×
      </button>
...
Header and footer are optional. The direct `.shlz-modal__surface` descendant is
required because it separates content hit-testing from the dialog backdrop.
Add `data-shlz-modal-backdrop-close` only when outside-pointer dismissal is safe.

## JavaScript contract

```js
import { enhanceModals, ModalController } from "@shlz/behaviors/modal";

const [modal] = enhanceModals();
modal.open();
modal.close("saved");
modal.destroy();
```

Buttons whose `data-shlz-modal-trigger` value matches the dialog ID are wired
declaratively. Descendant `[data-shlz-modal-close]` controls call native
`close()`; a non-empty attribute value becomes `dialog.returnValue`. The
controller listens to the native `close` lifecycle to synchronize
...
- `showModal()` supplies the top layer, modal semantics, background inertness,
  sequential-focus containment, Escape/cancel behavior and baseline initial
  focus. No custom trap or inert manager is installed.
- Use `autofocus` on a safe initial target when browser default focus ordering
  is not sufficient. Do not focus a destructive confirmation by default.
- `<form method="dialog">` is supported unchanged for native confirm/cancel
  return values.
...
Chromium browser tests cover Tab/Shift+Tab containment, Escape, explicit close,
focus return, opt-in backdrop dismissal, blocked background interaction,
native dialog forms, independently scrolling body content and `destroy()`.
Dropdown, Tooltip and Popover remain visible and receive the first Escape when
