...
- `axisLabels`: six non-empty, consumer-formatted tick strings, ordered from maximum to zero. An explicit `scaleMaximum` is required so labels remain correct when series visibility changes. This JSON-friendly alternative keeps locale/number formatting consumer-owned; omitted labels retain the legacy numeric fallback.
