...
The decorative cloud-upload icon uses source Gray 200 (`#939CA5`), observed on both paths in `Documents.svg`; the cloud silhouette retains the icon's source opacity of 0.4 and the arrow is opaque. Gray 75 belongs to the dashed boundary, not the icon. The same icon color is retained across available, disabled, error and drag-active states.
