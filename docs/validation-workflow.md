...
Use `review-init`, `review-record`, and `review-context` to persist the fixed base, reviewed head, axes, and findings. After a pass, `review-context` returns `<last-reviewed-head>..HEAD` plus unresolved findings, which is the complete re-review working set.
