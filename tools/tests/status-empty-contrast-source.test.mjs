...
test("Status source paints remain literal raw evidence and source tokens", async () => {
  const [status, tokensSource] = await Promise.all([
    read("shlz-design-source/raw/svg/Status.svg"),
...
test("Empty State source components remain in the Basic-elements archive", async () => {
  ...
  for (const [component, expectedPaint] of [
    ["Empty-Simple", "#939CA5"],
    ["Empty-Customize", "#0B1623"],
    ["Empty-Basic", "#939CA5"],
  ]) {
    const svg = readZipEntry(
      archive,
