...
test("Date Picker and Calendar authorities retain their attested integrity", async () => {
  for (const expected of sourceExpectations) {
    ...
    assert.match(root, new RegExp(`<svg width="${expected.canvas[0]}" height="${expected.canvas[1]}" viewBox="0 0 ${expected.canvas[0]} ${expected.canvas[1]}"`));
  }

  const references = JSON.parse(await readFile("design-source-index/reference-screens.json", "utf8"));
  const calendar = references.screens.find(({ sourcePath }) => sourcePath.endsWith("/Calendar.svg"));
...
test("indexed Date Picker primitives retain the attested source contract", async () => {
  const index = JSON.parse(await readFile("design-source-index/components.json", "utf8"));
  const named = (name) => index.components.filter((component) => component.name === name);
  ...
  assert.deepEqual(Object.keys(datePicker.propertyDefinitions), ["Size", "State", "Filled", "Ranged"]);
  assert.deepEqual(new Set(datePicker.variants.filter(({ properties }) => properties.State !== "Focused" && properties.State !== "Focused filled").map(({ properties, dimensions }) => `${properties.Size}:${dimensions.width}x${dimensions.height}`)), new Set(["Large:250x63", "Medium:250x55"]));

  const dropdowns = named("Picker-Dropdown");
  assert.deepEqual(dropdowns.map(({ figmaNodeId }) => figmaNodeId), ["89:5792", "89:13142"]);
  assert.deepEqual(dropdowns.map(({ dimensions }) => dimensions), [
  ...
  assert.ok(cell.variants.every(({ dimensions }) => dimensions.width === 30 && dimensions.height === 30));
  assert.deepEqual(Object.keys(cell.propertyDefinitions), ["In View", "Today", "Selected", "⬑ Range Start", "⬑ In Range", "⬑ Range End", "Hovered", "Disabled"]);

  const [icon] = named("Interface / Calendar");
  assert.equal(icon.kind, "COMPONENT");
  assert.deepEqual(icon.dimensions, { width: 24, height: 24 });
  assert.notEqual(icon.figmaNodeId, cell.figmaNodeId, "the calendar icon must not stand in for the calendar composition");
});

const roots = ["apps", "packages", "tools/fixtures"];
const extensions = new Set([".html", ".js", ".jsx", ".mjs", ".cjs", ".ts", ".tsx", ".vue", ".php"]);
const pickerOccurrence = /shlz-(?:calendar|date-picker)|data-(?:component-audit-id=["']date-picker-calendar-|shlz-(?:calendar|date-picker))|type\s*=\s*["']date["']/;
...
test("pre-implementation Date Picker census has no unclassified occurrence or substitute", async () => {
  const files = (await Promise.all(roots.map(executableFiles))).flat();
  const matches = [];
  for (const path of files) if (pickerOccurrence.test(await readFile(path, "utf8"))) matches.push(relative(".", path));
  assert.deepEqual(matches, []);

  const manifest = JSON.parse(await readFile("docs/component-audits/date-picker-calendar.json", "utf8"));
