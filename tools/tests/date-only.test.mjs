...
test("strict ISO validation accepts only real proleptic Gregorian dates", () => {
  ...
  for (const value of [
    ...
    "2026-8-28", "28-08-2026", "2026-08-28T00:00:00Z", "0000-01-01", "10000-01-01",
...
test("ISO comparison is chronological and rejects invalid operands", () => {
  ...
  assert.throws(() => compareIsoDates("2026-02-30", "2026-03-01"), /valid ISO date/i);
});
...
test("date-only arithmetic is invariant across process timezones", () => {
  const moduleUrl = new URL("../../packages/behaviors/src/date-only.ts", import.meta.url).href;
  ...
  const run = (TZ) => execFileSync(process.execPath, ["--input-type=module", "--eval", script], { encoding: "utf8", env: { ...process.env, TZ } }).trim();
...
test("localized parsing accepts only the complete active display pattern", () => {
  ...
  for (const value of ["28.8.2026", "28/08/2026", "08/28/2026", "28.08.26", "28.08.", "31.02.2026"])
    assert.equal(parseLocalizedDate(value, "ru-RU"), null, value);
  for (const value of ["8/28/2026", "28/08/2026", "08.28.2026", "02/30/2026", "08/28/"])
...
test("unsupported numbering systems fail explicitly instead of misparsing", () => {
  assert.throws(() => getLocalizedDatePattern("ar-EG"), /Latin decimal digits/i);
  assert.throws(() => parseLocalizedDate("٢٨‏/٠٨‏/٢٠٢٦", "ar-EG"), /Latin decimal digits/i);
});
