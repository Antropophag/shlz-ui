...
test("range selection remains provisional until an ordered second endpoint commits", () => {
  const initial = createCalendarState({
    mode: "range",
    value: { start: "2026-08-10", end: "2026-08-12" },
  });
  const replacement = selectCalendarDate(initial, "2026-08-28");
  assert.equal(replacement.committed, false);
  assert.deepEqual(replacement.state.value, {
    start: "2026-08-10",
    end: "2026-08-12",
  });
  ...
  assert.deepEqual(ordered.state.value, {
    start: "2026-08-20",
    end: "2026-08-28",
  });
...
test("min, max, and disabled dates prevent selection without mutating state", () => {
  ...
  assert.equal(
    selectCalendarDate(state, "2026-08-20", constraints).state.value,
    "2026-08-20",
  );
});
...
test("month navigation is bounded when the target month has no selectable date", () => {
  ...
  assert.equal(
    canNavigateCalendarMonth(state, -1, { min: "2026-08-01" }),
    false,
  );
  assert.equal(
    canNavigateCalendarMonth(state, 1, { max: "2026-08-31" }),
    false,
  );
  assert.deepEqual(
    navigateCalendarMonth(state, -1, { min: "2026-08-01" }),
    state,
  );
  assert.equal(
    navigateCalendarMonth(state, 1, { max: "2026-10-31" }).visibleMonth,
    "2026-09",
  );

  const allSeptemberDisabled = {
    isDateDisabled: (date) => date.startsWith("2026-09-"),
  };
...
test("dynamic constraints report mismatch without replacing committed consumer data", () => {
  ...
  assert.equal(
    getCalendarConstraintMismatch(single, { max: "2026-08-20" }),
    true,
  );
  assert.equal(single.value, "2026-08-28");

  const range = createCalendarState({
    mode: "range",
    value: { start: "2026-08-10", end: "2026-08-20" },
  });
  assert.equal(
    getCalendarConstraintMismatch(range, {
      isDateDisabled: (date) => date === "2026-08-20",
    }),
    true,
  );
...
test("invalid initial ranges and values fail at the public model boundary", () => {
  assert.throws(
    () => createCalendarState({ mode: "single", value: "2026-02-30" }),
    /valid ISO date/i,
  );
  assert.throws(
    () =>
...
test("arrow focus moves by days and weeks, crosses months, and skips disabled dates", () => {
  ...
  assert.equal(
    moveCalendarFocus("2026-08-28", "ArrowRight", {
      isDateDisabled: (date) => date === "2026-08-29",
    }),
    "2026-08-30",
  );
});
...
test("Home and End use the active locale week and skip unavailable boundary dates", () => {
  ...
  assert.equal(
    moveCalendarFocus(
      "2026-08-28",
      "Home",
      { isDateDisabled: (date) => date === "2026-08-24" },
      1,
    ),
    "2026-08-25",
  );
...
test("Page navigation clamps the day and skips disabled targets", () => {
  ...
  assert.equal(
    moveCalendarFocus("2026-01-31", "PageDown", {
      isDateDisabled: (date) => date === "2026-02-28",
    }),
    "2026-03-01",
  );
});
