...
test("production icons are generated byte-for-byte from normalized corpus", async () => {
  ...
  assert.equal(manifest.filter(({ currentColor }) => currentColor).length, 200);
  assert.equal(manifest.filter(({ currentColor }) => !currentColor).length, 44);
});
...
test("compatibility aliases resolve to normalized production SVGs", async () => {
  ...
  assert.ok(!aliases.some(({ alias }) => alias === "search-bold"));
...
test("repository validation follows the normalized icon corpus", async () => {
  const validator = await readFile("tools/validate.mjs", "utf8");
  assert.match(validator, /packages\/icons\/normalized\/manifest\.json/);
  assert.doesNotMatch(validator, /Expected 119 normalized logical icons/);
});
