...
const avatar = (size, type, iconUrl) => {
  ...
  const iconClass = type === "icon" ? " shlz-avatar--icon" : "";
  const textAttributes =
    type === "text" ? ` role="img" aria-label="User ${size}"` : "";
  return `<span class="shlz-avatar shlz-avatar--${size}${iconClass}" data-avatar-size="${size}" data-avatar-type="${type}" data-component-audit-id="avatar-showcase-${size}-${type}"${textAttributes}>${content}</span>`;
};

const linkMarkup = () => {
  const documentation = renderComponentDocumentation("link");
  return `<article id="link-demo"><h3>Link</h3>${documentation}<div class="shlz-cluster"><a class="shlz-link" href="#link-demo">Default</a><a class="shlz-link shlz-link--visual-hover" href="#link-demo">Hover</a><a class="shlz-link shlz-link--visual-pressed" href="#link-demo">Pressed</a><span class="shlz-link shlz-link--disabled">Disabled</span></div></article>`;
};

const avatarMatrixEntry = (size, iconUrl) => {
  const textAvatar = avatar(size, "text", iconUrl);
  const imageAvatar = avatar(size, "image", iconUrl);
  const iconAvatar = avatar(size, "icon", iconUrl);
  return `<span>${size}px</span>${textAvatar}${imageAvatar}${iconAvatar}`;
};

const avatarMarkup = (iconUrl) => {
  const documentation = renderComponentDocumentation("avatar");
  const matrix = [24, 32, 40, 64]
    .map((size) => avatarMatrixEntry(size, iconUrl))
    .join("");
  return `<article id="avatar-demo"><h3>Avatar</h3>${documentation}<p>Complete source matrix: four exact sizes × text, image and icon content.</p><div class="shlz-avatar-matrix"><b>Size</b><b>Text</b><b>Image</b><b>Icon</b>${matrix}</div></article>`;
};

const tableMarkup = (iconUrl) => {
  const sorter = tableSorter("Sort by name", "data-table-demo-sort");
  const filter = tableFilter(
    "Show Alpha only",
    'data-table-demo-filter aria-pressed="false"',
  );
  const priority = tablePriority("Priority");
  const assignedCellIcon = iconUrl("user-1");
  const avatarIcon = iconUrl("user-1");
  return `<article id="table-demo"><h3>Table</h3><p role="status" data-table-demo-result>Native table with application-owned sorting, filtering, selection and editing.</p><div class="shlz-table-wrap"><table class="shlz-table"><caption class="shlz-visually-hidden">Generic table foundation example</caption><thead class="shlz-table__head"><tr><th class="shlz-table__cell shlz-table__cell--check" scope="col"><input class="shlz-checkbox shlz-checkbox--sm" type="checkbox" aria-label="Select all rows"></th><th class="shlz-table__cell shlz-table__cell--priority" scope="col" aria-label="Priority">PRI</th><th class="shlz-table__cell" scope="col" aria-sort="ascending"><span class="shlz-table__heading"><span>Name</span><span class="shlz-table__actions">${sorter}${filter}</span></span></th><th class="shlz-table__cell" scope="col">Status</th><th class="shlz-table__cell" scope="col">Owner</th><th class="shlz-table__cell" scope="col">Mode</th><th class="shlz-table__cell" scope="col">Action</th></tr></thead><tbody><tr class="shlz-table__row"><td class="shlz-table__cell shlz-table__cell--check"><input class="shlz-checkbox shlz-checkbox--sm" type="checkbox" aria-label="Select Alpha"></td><td class="shlz-table__cell shlz-table__cell--priority">${priority}</td><td class="shlz-table__cell">Alpha request</td><td class="shlz-table__cell"><span class="shlz-status" data-component-audit-id="status-table-alpha">Active</span></td><td class="shlz-table__cell"><span class="shlz-avatar shlz-avatar--24" data-component-audit-id="avatar-table-alpha" role="img" aria-label="AP">AP</span></td><td class="shlz-table__cell shlz-table__cell--editable"><span class="shlz-dropdown shlz-table__cell-choice" data-shlz-dropdown data-table-menu-root data-component-audit-id="dropdown-table-standard"><button class="shlz-table__cell-choice-trigger" type="button" aria-haspopup="menu" aria-controls="table-mode-standard" aria-expanded="false">Standard</button><span class="shlz-dropdown__menu shlz-table__cell-choice-menu" role="menu" id="table-mode-standard" aria-label="Change mode" hidden><button class="shlz-dropdown__item" type="button" role="menuitem">Standard</button><button class="shlz-dropdown__item" type="button" role="menuitem">Review</button></span></span></td><td class="shlz-table__cell"><button class="shlz-button shlz-button--xs" type="button">Open</button></td></tr><tr class="shlz-table__row"><td class="shlz-table__cell shlz-table__cell--check"><input class="shlz-checkbox shlz-checkbox--sm" type="checkbox" aria-label="Select Beta" checked></td><td class="shlz-table__cell shlz-table__cell--icon"><img class="shlz-table__cell-icon" src="${assignedCellIcon}" alt="Assigned"></td><td class="shlz-table__cell shlz-table__cell--editable"><input class="shlz-table__editor" value="Beta request" aria-label="Edit name"></td><td class="shlz-table__cell"><span class="shlz-table__empty">—</span></td><td class="shlz-table__cell"><span class="shlz-avatar shlz-avatar--24 shlz-avatar--icon" data-component-audit-id="avatar-table-beta" aria-hidden="true"><img class="shlz-avatar__icon" src="${avatarIcon}" alt=""></span></td><td class="shlz-table__cell shlz-table__cell--editable"><span class="shlz-dropdown shlz-table__cell-choice" data-shlz-dropdown data-table-menu-root data-component-audit-id="dropdown-table-review"><button class="shlz-table__cell-choice-trigger" type="button" aria-haspopup="menu" aria-controls="table-mode-review" aria-expanded="false">Review</button><span class="shlz-dropdown__menu shlz-table__cell-choice-menu" role="menu" id="table-mode-review" aria-label="Change mode" hidden><button class="shlz-dropdown__item" type="button" role="menuitem">Standard</button><button class="shlz-dropdown__item" type="button" role="menuitem">Review</button></span></span></td><td class="shlz-table__cell"><button class="shlz-button shlz-button--primary shlz-button--xs" type="button">Save</button></td></tr></tbody></table></div></article>`;
};

export const wave3Markup = (iconUrl) =>
  ["", linkMarkup(), avatarMarkup(iconUrl), tableMarkup(iconUrl)].join("\n");
