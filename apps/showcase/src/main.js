import { tableCellsMarkup, initTableCells } from "./table-cells.js";
import { tableCompositionsMarkup } from "./table-compositions.js";
import "@shlz/styles";
...
import { wave3Markup, enhanceTableDemo } from "./wave3.js";
...
const canonicalIconNames = new Set(manifest.map(({ name }) => name));
...
const relatedByTarget = groupBy(
  manifest.filter(({ name }) => relatedTargetByName.has(name)),
  ({ name }) => relatedTargetByName.get(name),
);
const catalogManifest = manifest.filter(
  ({ name }) => !relatedTargetByName.has(name),
);
const iconGroups = [...groupBy(catalogManifest, ({ category }) => category)]
  .map(([category, icons]) => {
...
<dialog class="shlz-modal shlz-modal--compact" id="showcase-confirm" data-shlz-modal data-component-audit-id="modal-showcase-compact" aria-labelledby="showcase-confirm-title"><form class="shlz-modal__surface shlz-modal__surface--compact shlz-modal__surface--info" method="dialog"><div class="shlz-modal__compact-content"><span class="shlz-modal__variant-icon" aria-hidden="true">i</span><div class="shlz-modal__compact-copy"><h2 id="showcase-confirm-title">Подтвердить действие?</h2><p>Для confirm/cancel доступна нативная форма <code>method=&quot;dialog&quot;</code>.</p></div></div><div class="shlz-modal__compact-actions"><button class="shlz-button shlz-button--sm" value="cancel">Отмена</button><button class="shlz-button shlz-button--primary shlz-button--sm" value="confirm">Подтвердить</button></div></form></dialog></article>
...
<section id="icons" class="shlz-icon-catalog"><h2>Icons <span class="shlz-evidence" data-kind="FACT">FACT · normalized authoritative icon manifest</span></h2><p>${catalogManifest.length} visual icon families from ${manifest.length} canonical names. Source variants and compatibility aliases are consolidated as metadata instead of repeated cards.</p><div data-icon-catalog-content></div></section></section>
...
enhanceTableDemo();
window.__shlzTableCells = initTableCells(document, { enhanceDropdowns });
...
// Keep the opt-in catalog after existing visual fixtures so expanding a large
// authoritative corpus cannot shift their subpixel screenshot coordinates.
app.append(document.querySelector("#icons"));

const hydrateIconCatalog = () => {
  if (window.location.hash !== "#icons") return;
  const content = document.querySelector("[data-icon-catalog-content]");
  if (content && !content.childElementCount) content.innerHTML = iconGroups;
};
hydrateIconCatalog();
window.addEventListener("hashchange", hydrateIconCatalog);
