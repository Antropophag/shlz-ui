...
export const primaryComponentMarkup = `
  ...
  <article class="shlz-api-component" id="select-demo">
    ...
    <section data-select-production-fixtures><h4 class="shlz-select-fixture-label">Production native single-select</h4><p class="shlz-select-fixture-label">Executable closed-field states. The opened option list remains browser-owned.</p><div class="shlz-component-grid"><div>${selectField({ label: "Статус заявки", placeholder: "Выберите статус", value: "В работе" })}</div><div>${selectField({ label: "Статус заявки", filled: true, value: "В работе" })}</div><div>${selectField({ label: "Наведение", state: "visual-hover", filled: true, value: "Ожидает ответа" })}</div><div>${selectField({ label: "Фокус", state: "visual-focus", filled: true, value: "Назначена" })}</div><div>${selectField({ label: "Недоступно", state: "disabled", filled: true, value: "Закрыта" })}</div><div>${selectField({ label: "Компактный размер", size: "medium", filled: true, value: "В работе" })}</div></div></section>
