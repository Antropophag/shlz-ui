...
) => `<div class="shlz-document-row${compact ? " shlz-document-row--compact" : ""}" data-file-type="${type}">
...
<article id="file-row-extension-demo" data-shlz-visual-addition><h3>Document Row · SHLZ extension</h3><p>A fluid document-list composition with an independent file visual, content column, and stable action area. The authoritative File Row above remains unchanged.</p><div class="shlz-document-row-showcase">
  <section class="shlz-document-row-showcase__primary"><h4>Metadata-rich list · fluid container</h4><div class="shlz-document-list shlz-document-row-visual-fixture">
    ${documentRow("Сопроводительные материалы 100.pdf", "pdf-default", iconUrl)}
    ${documentRow("Очень длинное название корпоративного документа с приложениями и дополнениями.docx", "docx", iconUrl, { modified: "10.08.2026, 09:12", size: "248 КБ" })}
    ${documentRow("Бюджет проекта.xlsx", "xlsx", iconUrl, { modified: "08.08.2026, 17:40", version: "Версия 4", size: "1,2 МБ" })}
  </div></section>
  <section class="shlz-document-row-showcase__compact"><h4>Compact list</h4><div class="shlz-document-list">
    ${documentRow("План.pdf", "pdf-default", iconUrl, { compact: true })}
    ${documentRow("Протокол рабочей встречи.docx", "docx", iconUrl, { compact: true, size: "86 КБ" })}
    ${documentRow("Расчёты.xlsx", "xlsx", iconUrl, { compact: true, version: "Версия 2", size: "740 КБ" })}
  </div></section>
</div></article>
