...
${comment({ author: "Александр Васильев", initials: "АВ", time: "4 дня назад", id: "comment-source-one", body: "Коллеги, всем привет! Предлагаю обсудить здесь текущую ситуацию по этой заявке. Также отправляю несколько важных документов для ознакомления.", files: file("схема-лифт47.pdf", "31 MB", "file-row-comment-source-pdf") + file("lift_illustration.docx", "9 MB", "file-row-comment-source-docx"), summary: '2 файла, <span class="shlz-comment-feed__file-meta">40 MB</span> <a class="shlz-link shlz-comment-feed__download-all" data-component-audit-id="link-comment-feed-download-all" href="#comment-feed-demo">Скачать все</a>' })}
${comment({ author: "Александр Васильев", initials: "АВ", time: "3 дня назад", id: "comment-source-two", body: "Также прикрепляю договор с клиентом", files: file("Договор Рязань ли…", "12 MB", "file-row-comment-source-contract"), summary: '1 файл, <span class="shlz-comment-feed__file-meta">12 MB</span>' })}
${comment({ author: "Михаил Богданов", initials: "МБ", time: "1 день назад", id: "comment-source-three", body: "Если гарантийный срок на лифт еще действует, следует оформить претензию производителю для компенсации затрат на ремонт." })}
${comment({ author: "Александр Васильев", initials: "АВ", time: "1 день назад", id: "comment-source-four", mention: "Михаил Богданов", body: "Гарантийный срок не действует. Рекомендуется установить срок выполнения работ в течение 5–7 рабочих дней." })}
</ol>`;
...
const stateComment = (context = "") =>
  `<ol class="shlz-comment-feed" aria-label="Фрагмент комментария">${comment({ author: context === "reply" ? "Михаил Богданов" : "Александр Васильев", initials: context === "reply" ? "МБ" : "АВ", time: "1 день назад", body: "Комментарий в выбранном состоянии.", context })}</ol>`;
const state = (name, body) =>
  `<section data-comment-feed-state="${name}"><h4>${name}</h4><div class="shlz-comment-feed__surface">${body}</div></section>`;

export const commentFeedShowcaseMarkup = `<article id="comment-feed-demo"><h3>Comment Feed</h3><p><code>Комментарии.svg</code> · source fixture; data and mutations are consumer-owned.</p>${state("default", comments("comment-feed-showcase-source") + composer({ audited: true }))}<details class="shlz-component-diagnostics"><summary>Source-observed states</summary><div class="shlz-stack">${state("composer-populated", composer({ populated: true }))}${state("comment-added", stateComment() + '<div class="shlz-notification" role="status"><div class="shlz-notification__content"><p class="shlz-notification__title">Комментарий успешно добавлен</p></div></div>')}${state("own-comment-actions", stateComment("own"))}${state("other-comment-reply", stateComment("reply"))}${state("mention-suggestions", composer({ populated: true, suggestions: true }))}${state("comment-deleted", '<div class="shlz-notification"><span class="shlz-notification__countdown">4</span><div class="shlz-notification__content"><p class="shlz-notification__title">Комментарий удален</p></div><button class="shlz-notification__action" type="button">Отменить</button></div>')}</div></details></article>`;
