...
When apply re-enters requirements, follow `docs/requirements-elicitation.md`: the resolved user answer or explicit delegation is confirmation for the corresponding artifact revision, so update it without asking the same decision again; unrelated revisions still require confirmation.
