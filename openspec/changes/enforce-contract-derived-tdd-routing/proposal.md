...
The public harness `plan` command, plan construction/validation modules, OpenSpec delta parsing, harness fixtures/tests, and execution/validation documentation are affected. No design-system package or `shlz-design-source/` file changes. The principal risk is over-classifying evidence-only scenarios; the closed semantics vocabulary and exclusion fixtures bound that risk. PR merge remains user-owned and outside this change.
