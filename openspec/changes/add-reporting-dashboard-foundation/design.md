...
See `proposal.md`. The current library already supplies most controls needed by tabular reports and a source-backed Report Card, but it has no dashboard layout, widget surface, or independently audited metric composition. The authoritative evidence is split across two component-sheet/screen naming families (`Dashboard.svg` and `Дашборды.svg`), an editing screen, four report workflow screens, and `Reports card.svg`; their text is outlined, so geometry and paint are more recoverable than literal labels or domain semantics.
...
The public layer has three levels: Dashboard arranges sections, Widget provides a semantic content surface, and Metric Card supplies the narrow value-summary composition. Metric Card may share source-backed variables with Report Card, but Report Card remains compatible and neither class becomes an alias for the other unless the ledger proves identical structure.
...
### 5. Treat chart-shaped regions as unresolved

The ledger records plot areas, legends, axes, and graphical marks when observed, but this change does not emit chart APIs or decorative approximations. A later chart capability must resolve data/series semantics, responsive rules, interaction, accessible alternatives, and exact visual families.

### 6. Complete each new family independently

Dashboard, Widget, and Metric Card receive separate audit manifests, stable occurrence IDs, focused fixtures, a real reporting consumer, and browser evidence. Computed-style assertions protect source-critical geometry and paint; component-local screenshots protect composition; controlled mutations demonstrate that the oracles detect material drift.
...
- [Metric Card could duplicate Report Card] → Compare their ledger regions and share only proven internal values while retaining explicit public purposes.
