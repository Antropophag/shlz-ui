...
A passing validation receipt for a declared finite set MUST partition every member into exactly one of executable-covered or explicitly excluded. Each covered member MUST reference executable outcome evidence in the validation input closure. Each excluded member MUST carry a non-empty justification and MUST NOT be reported as covered.
...
- **THEN** validation emits a receipt bound to the normalized declaration, coverage, exclusions, and evidence references
