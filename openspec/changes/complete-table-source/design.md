...
- Place expanded source galleries after existing Showcase sections and mark them as additive diagnostics. Keep table feedback in its existing description slot, and bind positioned hidden labels to the table wrapper, so new examples do not shift legacy snapshot origins or create page-wide overflow.
