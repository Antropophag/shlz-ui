...
- [x] 1.1 Record the clean Wave 11 baseline, current CI/review state, authoritative source hashes, named composition boundaries, and source-critical facts; verify both raw SVGs remain unchanged and every claim has an explicit classification.

## 2. Manifest and occurrence census

- [x] 2.1 Add the Upload / Document compositions schema-v2 manifest with per-source and per-variant ledgers, explicit File Row / Document Row dependency boundaries, exact evidence applicability, findings, and limitations; verify manifest contract tests pass.
- [x] 2.2 Add a focused repository-wide census for higher-level composition signatures, primitive-only surfaces, and unrelated terminology; verify the current classification passes and a synthetic matching surface fails until classified.
